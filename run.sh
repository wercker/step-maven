...
if [[ -z "$WERCKER_STEP_MAVEN_GOALS" ]]; then
...
#
if [[ -z "$WERCKER_STEP_MAVEN_VERSION" ]]; then
  WERCKER_STEP_MAVEN_VERSION="3.5.0"
fi
...
if [ ! -d "/maven" ]; then
  ...
  CHECK1=$(cat apache-maven-$WERCKER_STEP_MAVEN_VERSION-bin.tar.gz.md5)
  CHECK2=$(md5sum apache-maven-$WERCKER_STEP_MAVEN_VERSION-bin.tar.gz)
  CHECK1="$CHECK1  apache-maven-$WERCKER_STEP_MAVEN_VERSION-bin.tar.gz"
  ...
  if [ ! -x "/maven/apache-maven-$WERCKER_STEP_MAVEN_VERSION/bin/mvn" ] ; then
...
if [ "$WERCKER_STEP_MAVEN_DEBUG" = "true" ]; then
  DEBUG="-X -e"
else
  DEBUG=""
fi

if [[ -z "$WERCKER_STEP_MAVEN_PROFILES" ]]; then
  PROFILES=""
else
  PROFILES="-p $WERCKER_STEP_MAVEN_PROFILES"
fi

if [[ -z "$WERCKER_STEP_MAVEN_MAVEN_OPTS" ]]; then
  MAVEN_OPTS=""
else
  MAVEN_OPTS="-p $WERCKER_STEP_MAVEN_MAVEN_OPTS"
fi

if [[ -z "$WERCKER_STEP_MAVEN_SETTINGS" ]]; then
  SETTINGS=""
else
  SETTINGS="-s $WERCKER_STEP_MAVEN_SETTINGS"
fi
